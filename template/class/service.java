			...
			for (String key : jo.keySet()) {
				if(jo.get(key)!=null&&StringUtils.isNotEmpty(jo.get(key).toString())){
					if(jo.get(key) instanceof String){
						sql.append("and ${className}.").append(key).append(" like ? ");
						...
						if(json.get("id")==null || json.getInteger("id")==-1){
							...
							sql.append("and ${className}.").append(key).append(".id=? ");
							params.add(json.get("id"));
						}
					}else{
						sql.append("and ${className}.").append(key).append("=? ");
						params.add(jo.get(key));
					}
				}
			}
			JSONArray ja = po2JsonList(${className}Dao.findInPage(String.format("from ${classNameB} ${className} %s", sql), params.toArray(), 1, 10).getList(),false);
